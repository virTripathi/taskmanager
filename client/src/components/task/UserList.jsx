...
const UserList = ({ setAssignedTo, assignedTo }) => {
  // Fetch user data
  ...
  const handleChange = (el) => {
    setSelectedUser(el);
    setAssignedTo(el ? [el.id] : []);
  };

  const lastSelectedUserName = selectedUser?.name || "Select Users";
        ...
        <Listbox value={selectedUser} onChange={handleChange}>
          <div className='relative mt-1'>
            <Listbox.Button className='relative w-full cursor-default rounded bg-white pl-3 pr-10 text-left px-3 py-2.5 2xl:py-3 border border-gray-300 sm:text-sm'>
              <span className='block truncate'>{lastSelectedUserName}</span>
